export function createMaxTools(osc) {
    return [
        {
...
export async function executeMaxTool(osc, toolName, toolInput) {
    try {
        switch (toolName) {
            ...
            case 'max_send_message': {
                ...
                const deviceIndex = toolInput.deviceIndex;
                ...
                if (!deviceIndex && deviceIndex !== 0)
                    throw new Error('Device index required');
                if (!message)
            ...
            case 'max_get_parameter': {
                const trackIndex = toolInput.trackIndex;
                const deviceIndex = toolInput.deviceIndex;
                const parameterName = toolInput.parameterName;
                if (!trackIndex && trackIndex !== 0)
                    throw new Error('Track index required');
                if (!deviceIndex && deviceIndex !== 0)
                    throw new Error('Device index required');
                ...
                const value = await osc.request(`/live/track/${trackIndex}/devices/${deviceIndex}/max/parameters/${parameterName}/value`);
