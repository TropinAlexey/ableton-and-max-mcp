export function createNotesTools(osc) {
    return [
        {
            name: 'notes_get',
...
function generateArpeggio(root, scale, length, direction) {
    const notes = [];
...
function generateChord(root, scale, length) {
    const notes = [];
    const scaleNotes = getScaleNotes(root, scale);
    const beatPerBar = 4;
    const totalBeats = length * beatPerBar;
...
export async function executeNotesTool(osc, toolName, toolInput) {
    try {
        const trackIndex = toolInput.trackIndex;
        const sceneIndex = toolInput.sceneIndex;
        switch (toolName) {
            case 'notes_get':
            ...
            case 'notes_add':
                ...
                {
                    const pitch = toolInput.pitch;
                    const startTime = toolInput.startTime;
                    const duration = toolInput.duration;
                    const velocity = toolInput.velocity;
            ...
            case 'notes_generate_pattern': {
                ...
                const root = toolInput.root;
                const scale = toolInput.scale;
                const length = toolInput.length;
                let generatedNotes = [];
