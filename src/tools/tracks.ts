...
export function createTracksTools(osc: AbletonOSCClient): Tool[] {
  return [
    {
      name: 'tracks_list',
...
export async function executeTracksTool(
  ...
  try {
    ...
    switch (toolName) {
      ...
      case 'tracks_create_audio': {
        const name = toolInput.name as string || 'New Track';
      ...
      case 'tracks_delete':
        if (index === undefined) throw new Error('Track index required');
      ...
      case 'tracks_set_volume':
        if (index === undefined) throw new Error('Track index required');
        {
          const volume = toolInput.volume as number;
      ...
      case 'tracks_set_pan':
        if (index === undefined) throw new Error('Track index required');
        {
          const pan = toolInput.pan as number;
          if (pan < -1 || pan > 1) {
